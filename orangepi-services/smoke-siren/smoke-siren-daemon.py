...
# ===========================================================================
# 🔴 DIMMING IS THRESHOLD-BASED ON/OFF, NOT PWM - and that is deliberate, not
# an oversight. Real software PWM was tried 2026-08-07/08 (a dedicated
# RgbPwm thread, 200Hz), deployed, and the user reported live "очень сильная
# пульсация и неравномерная причем. мигания" (strong, uneven flicker).
# Extensive on-device measurement could not conclusively root-cause it (the
# signal was mostly clean but had one irregular burst in one recording;
# CPU-runaway was ruled out). The user's own call: probably some combination
# of the MOSFET/transistor switching characteristics and digital timing
# jitter - not worth chasing further, revert to plain on/off. Reverted
# 2026-08-08. Full investigation and the revert decision:
# docs/printer-status.md, section "Живая жалоба на мерцание после деплоя
# PWM" - read that before re-attempting PWM on this hardware.
#
...
#   🔴 STALE AS OF 2026-08-08 - FLAGGED, NOT RESOLVED: this "not wired" claim
#   dates to 2026-08-07. The user's live flicker report (2026-08-07/08, see
#   docs/printer-status.md "Живая жалоба на мерцание после деплоя PWM") is
#   direct evidence a physical strip WAS on these pins and switching by that
#   point - the two statements contradict each other and neither was
#   reconciled during the PWM revert. Treat the physical wiring state as
#   UNCONFIRMED by this file and verify at the machine before trusting
#   either claim; don't silently pick one.
#
