 ...
 *
 * 🔴 ЛОВУШКА --set: ТИП ЗНАЧЕНИЯ ОБЯЗАН СОВПАДАТЬ С ФОРМОЙ, В КОТОРОЙ КЛЮЧ ЛЕЖИТ
 * В ПРЕСЕТЕ. Профили OrcaSlicer стрингово типизированы: скаляры это СТРОКИ,
 * по-экструдерные настройки — массивы строк. `--set process.travel_acceleration=9999`
 * (голое число) — валидный JSON, поэтому скрипт его молча примет, но слайсер такой
 * ключ не разберёт и подставит СВОЙ дефолт. Наблюдалось живьём 2026-08-18: с
 * `=9999` в gcode вышло `travel_acceleration = 10000` (встроенный дефолт Orca), а
 * не 9999 и даже не 7000 из пресета; с `="9999"` — ровно 9999. Ошибки при этом нет
 * ни одной, а 10000 выглядит достаточно правдоподобно, чтобы его приняли за
 * результат опыта. Всегда писать `="9999"` / `=["1500","1500"]`, и ПРОВЕРЯТЬ, что
 * значение доехало: в хвосте готового gcode слайсер печатает весь эффективный
 * конфиг комментариями — `grep '^; travel_acceleration ' out/plate_1.gcode`.
 *
