...
# Бэкапы — в поддиректорию $REMOTE_DIR, с ротацией (2026-08-16).
...
# среди залежей .bak-файлов.
# Первая версия этой правки клала бэкапы в СОСЕДНЮЮ printer_data/config-backups/
# (не подкаталог) специально чтобы Mainsail их не показывал вообще - но
# пользователь явно попросил (2026-08-16) видеть папку бэкапов в интерфейсе,
# так что теперь это подкаталог $REMOTE_DIR/backups - Mainsail's file_manager
# "config" root это и есть $REMOTE_DIR, подкаталог внутри него попадает в тот
# же браузер как вложенная папка, что и требовалось. НЕ .backups (со скрытой
# точкой) - большинство файловых браузеров прячут dotfiles по умолчанию, что
# свело бы на нет саму цель "видеть папку".
