...
Само переключение темы (вызов `change_theme()`, таймер, реакция на смену) - в
следующей фазе, здесь его нет.
...
### ПРЕДУСЛОВИЕ ДЛЯ СЛЕДУЮЩЕЙ ФАЗЫ: без патча config.py киоск не поднимется
...
Патч в `klipperscreen-patches/` намеренно НЕ заведён: у той папки контракт
"root cause + диф + подтверждение живьём", а подтвердить живьём в этой фазе
нечем - она устройства не касается. Заводить вместе с фазой переключения.

До тех пор модуль работает на дефолтах (Москва) и конфиг не трогает - это
безопасно, потому что отсутствие опций в `[main]` ошибкой не является.
